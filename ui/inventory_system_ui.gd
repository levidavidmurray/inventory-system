...
func _ready():
	...
	if other_craft_station_ui.input_inventory_ui != null:
		...
		other_craft_station_ui.input_inventory_ui.inventory_point_down.connect(
			_inventory_point_down.bind()
		)
	if other_craft_station_ui.output_inventory_ui != null:
		...
		other_craft_station_ui.output_inventory_ui.inventory_point_down.connect(
			_inventory_point_down.bind()
		)
...
func _slot_point_down(event: InputEvent, slot_index: int, inventory: Inventory):
	...
	if inventory_handler.is_transaction_active():
		var amount = _get_amount_per_mouse_event(
			mouse_event, inventory_handler.transaction_slot.amount
		)
